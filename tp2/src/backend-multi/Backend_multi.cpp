...
unsigned int alto = -1;
RWLock escribiendo;
vector<vector<char> > tablero_usadas_provisorias;
vector<vector<RWLock> > tablero_de_mutexes;
...
int main(int argc, const char* argv[]) {
    ...
    tablero_usadas_provisorias = vector<vector<char> >(alto);
    for (unsigned int i = 0; i < alto; ++i) {
        tablero_usadas_provisorias[i] = vector<char>(ancho, VACIO);
    }

    tablero_de_mutexes = vector<vector<RWLock> >(alto);

    for (unsigned int i = 0; i < alto; ++i) {
        tablero_de_mutexes[i] = vector<RWLock>(ancho, RWLock());
    }
    ...
    while (true) {
        if ((socketfd_cliente = accept(socket_servidor, (struct sockaddr*) &remoto, (socklen_t*) &socket_size)) == -1)
            ...
            pthread_t tipito;
            int zonzos = pthread_create(&tipito, (const pthread_attr_t*)0,(void* (*) (void*))atendedor_de_jugador, (void*) (long) socketfd_cliente);
            if (!zonzos){
                cerr << "Error! Murio todo. FORROS!" << endl;
            }
        }
    }
    return 0;
